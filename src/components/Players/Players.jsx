...
const Players = ({ coins, setCoins }) => {
    ...
    const handleSelectedPlayer = (player) => {
        ...
        if (coins < parseInt(player.biddingPrice.replace('$','').replace(/,/g,''))) {
            toast.error(`You need more coins to select ${player.name}!`);
            return;
        }

        setCoins(coins - parseInt(player.biddingPrice.replace('$','').replace(/,/g,'')));
    ...
    return (
        <div className="container mx-auto flex flex-col justify-between items-center p-4 md:p-6">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center py-4 px-4 md:px-6 w-full md:w-5/6 mx-auto">
                ...
                <div>
                    <button
                        className={`btn ${isAvailable ? 'bg-yellow-300' : 'bg-slate-200'} block md:inline-block px-6 md:px-10 py-3 md:rounded-l-lg font-semibold mt-6 md:mt-0`}
                        onClick={() => handleToggleView('available')}
                    >
                        Available
                    </button>
                    <button
                        className={`btn ${!isAvailable ? 'bg-yellow-300' : 'bg-slate-200'} block md:inline-block px-4 md:px-8 py-3 rounded-r-lg font-semibold mt-3 md:mt-0`}
                    ...
                    {players.map((player) => (
                        <div key={player.playerId} className="card bg-base-100 shadow-xl rounded-lg">
                            <figure className="px-4 pt-6">
                                <img
                                    className="w-full h-30 md:h-full rounded-xl"
                                    src={player.image}
                                    alt={`Image of player ${player.name}`}
                                />
                            </figure>
                            <div className="card-body">
                                ...
                                <p className="text-gray-600 text-base flex gap-2 ml-4 md:ml-10 mt-4 mb-3">
                            ...
                            <div className="card-actions mt-4">
                                <button
                                    className="btn border rounded-lg bg-slate-100 px-6 md:px-8 text-lg md:text-xl mt-2 ml-4 md:ml-10 mb-6 md:mb-10 font-medium p-2"
                        ...
                        <div>
                            <ul>
                                {selectedPlayers.map((player) => (
                                    <li
                                        key={player.playerId}
                                        className="flex justify-between items-center bg-white p-4 border rounded-lg mb-4"
                                    >
                                        <div className="flex items-center gap-4">
                                            <img
                                                className="w-16 h-16 rounded-full"
                                                src={player.image}
                                                alt={`Image of selected player ${player.name}`}
                                            />
                                            <div>
                                        ...
                                        <button onClick={() => handleDeletePlayer(player.playerId)} aria-label={`Delete ${player.name}`}>
                            ...
                            <button
                                className="btn bg-yellow-300 px-8 py-3 rounded-lg font-bold"
                                onClick={() => handleToggleView('available')}
                            >
                                Add More Player
                            </button>
